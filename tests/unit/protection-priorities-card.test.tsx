...
describe("ProtectionPrioritiesCard — the top attention areas on the home", () => {
    it("speaks in the app's plural voice, carries no score, no dialog and no upload action of its own", () => {
        ...
        // B4: every count on the card is a door (a[data-count]); the ACTION link is still exactly one.
        expect(container.querySelectorAll("a[data-count]").length).toBeGreaterThan(0)
        expect(container.querySelectorAll("a:not([data-count])")).toHaveLength(1)
        expect(container.querySelector("a:not([data-count])")?.getAttribute("href")).toBe("/wallet/add")
    ...
    it("absence is never evidence — the caveat renders with and without policies, and the two footer lines are separate", () => {
        ...
        expect(none.container.querySelector("a:not([data-count])")?.textContent).toContain("Δείτε τι λένε τα ασφαλιστήριά σας")
        ...
        expect(some.container.querySelector("a:not([data-count])")?.getAttribute("href")).toBe("/protection?lens=risk")
    })
    ...
    it("its one link records what the person set out to do", () => {
        ...
        fireEvent.click(first.container.querySelector("a:not([data-count])")!)
        ...
        fireEvent.click(second.container.querySelector("a:not([data-count])")!)
...
describe("ProtectionProfileResumeCard", () => {
    it.each(["in_progress", "start"] as const)("%s is one link into /onboarding", (variant) => {
        ...
        const link = container.querySelector("a:not([data-count])")
