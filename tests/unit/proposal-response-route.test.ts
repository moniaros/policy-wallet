...
describe('PATCH proposal response — decline', () => {
    ...
    // Merge note (2026-08-01): this branch coerced an unrecognised reason to
    // `other` at runtime, but NEW-UI had meanwhile added Zod validation on the
    // request body — so an unknown reason is now rejected at the boundary and
    // never reaches the mapper. The strict boundary is the behavior we keep
    // (Zod-validated input is the house rule); lostOutcomeFromDeclineReason
    // stays as defense in depth and is unit-tested directly in
    // opportunity-lifecycle.test.ts. Both halves are pinned below.
    it('REJECTS an unrecognised reason at the boundary — no partial write', async () => {
        mockOppFind.mockResolvedValue({ id: 'opp-1', status: 'open' } as any)

        const res = await (PATCH as any)(ctx({ status: 'declined', declineReason: 'mystery' }))

        expect(res.status).toBe(400)
        // Nothing may be written on a rejected payload — not the proposal, not
        // the deal, not the history row.
        expect(tx.proposal.update).not.toHaveBeenCalled()
        expect(tx.opportunity.update).not.toHaveBeenCalled()
        expect(tx.opportunityStageHistory.create).not.toHaveBeenCalled()
    })

    it('closes the deal as `other` when a decline carries no reason at all', async () => {
        // declineReason is optional, so this IS reachable — it is the live path
        // through lostOutcomeFromDeclineReason's null branch.
        mockOppFind.mockResolvedValue({ id: 'opp-1', status: 'open' } as any)

        await (PATCH as any)(ctx({ status: 'declined' }))
