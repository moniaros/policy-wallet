        ...
        switch (gap.priority) {
            case 'high':
                return {
                    ...
                    label: language === 'el' ? 'Υψηλή Προτεραιότητα' : 'High Priority'
                }
            case 'medium':
                return {
                    ...
                    label: language === 'el' ? 'Μέτρια Προτεραιότητα' : 'Medium Priority'
                }
            case 'low':
                return {
                    ...
                    label: language === 'el' ? 'Χαμηλή Προτεραιότητα' : 'Low Priority'
                }
        }
    }
        ...
        <div className={`border-2 rounded-2xl p-4 ${styles.container}`}>
            ...
            {gap.estimatedCost && (
                <p className="text-xs text-stone-600 dark:text-stone-400 mb-3">
                    {language === 'el' ? 'Εκτιμώμενο κόστος από' : 'Estimated from'} <span className="font-bold">{formatCurrency(gap.estimatedCost)}</span>{language === 'el' ? '/έτος' : '/year'}
                </p>
            )}
            ...
            <button
                ...
                aria-label={`${language === 'el' ? 'Προσθήκη κάλυψης για' : 'Add coverage for'} ${gap.title}`}
            >
                <PlusIcon className="w-5 h-5" />
                {language === 'el' ? 'Προσθήκη Κάλυψης' : 'Add Coverage'}
            </button>
        </div>
    )
}
