...
export function RecommendedActionsWidget({
    ...
    return (
        <div className="pw-card pw-pad lg:col-span-2">
            ...
            <div className="mt-3">
                {items.length === 0 ? (
                    <p className="text-sm text-black/55 dark:text-white/65">{labels.noActions}</p>
                ) : (
                    <div className="space-y-2">
                        {items.map((item) => (
                            ...
                            >
                                ...
                                <ArrowRight className="h-3.5 w-3.5 flex-shrink-0 text-black/40 dark:text-white/45" aria-hidden />
                            </Link>
                        ))}
                    </div>
                )}
