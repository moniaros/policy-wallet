...
const INPUT_CLASS = 'w-full h-12 px-5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl focus:ring-4 focus:ring-primary/10 outline-none transition-all text-sm font-bold'
                ...
                <div className="p-12">
                    ...
                    {view === 'upload' && (
                        <div className="space-y-10">
                            ...
                            <button
                                ...
                                className="w-full p-12 rounded-[40px] border-2 border-dashed border-stone-200 dark:border-stone-700 hover:border-primary dark:hover:border-mint transition-all bg-stone-50/50 dark:bg-stone-800/30 flex flex-col items-center gap-4 group"
                            >
                                ...
                                <div className="text-center">
                                    <p className="text-base font-black text-stone-900 dark:text-white">{up.uploadCta}</p>
                                    <p className="text-xs text-stone-400 font-medium mt-1">{up.dropHint}</p>
                                    <p className="text-[10px] text-stone-400 font-medium mt-2 uppercase tracking-widest">{up.uploadHint}</p>
                                </div>
                            ...
                            <button onClick={closeAll} className="w-full py-4 text-[10px] font-black uppercase tracking-widest text-stone-400 hover:text-stone-900 transition-colors">
                                {up.cancel}
                            </button>
                        </div>
                    )}
                    ...
                    {view === 'parsing' && (
                        <div className="py-20 text-center">
                            ...
                            <h2 className="text-2xl font-black text-stone-900 dark:text-white tracking-tighter mb-2">{up.analyzingTitle}</h2>
                            <p className="text-stone-400 font-medium">{up.analyzingDesc}</p>
                        </div>
                    )}
                    ...
                    {view === 'resolve' && resolution && (
                        <div className="space-y-8">
                            <header>
                                ...
                                <h2 className="text-2xl font-black text-stone-900 dark:text-white tracking-tighter mt-2">
                                    {up.resolveTitle} <span className="text-stone-400 italic">{up.resolveAccent}</span>
                                </h2>
                            </header>
                            ...
                            <div className="p-6 rounded-[28px] bg-stone-50 dark:bg-stone-800/40 border border-stone-100 dark:border-stone-800">
                                <p className="text-[10px] font-black uppercase tracking-widest text-stone-400 mb-3">{up.extractedTitle}</p>
                                <div className="grid grid-cols-2 gap-3 text-sm">
                                    <div><span className="text-stone-400 font-medium">{up.nameLabel}: </span><span className="font-bold text-stone-900 dark:text-white">{[customer.name, customer.surname].filter(Boolean).join(' ') || '—'}</span></div>
                                    <div><span className="text-stone-400 font-medium">{up.afmLabel}: </span><span className="font-bold text-stone-900 dark:text-white">{customer.taxId || '—'}</span></div>
                                    <div><span className="text-stone-400 font-medium">{up.emailLabel}: </span><span className="font-bold text-stone-900 dark:text-white">{customer.email || '—'}</span></div>
                                    <div><span className="text-stone-400 font-medium">{up.phoneLabel}: </span><span className="font-bold text-stone-900 dark:text-white">{customer.phone || '—'}</span></div>
                                </div>
                            </div>
                            ...
                            {resolution.candidates.length === 0 ? (
                                <p className="text-sm font-bold text-stone-900 dark:text-white">
                                    {up.newCustomerTitle}. <span className="font-medium text-stone-500">{up.newCustomerDesc}</span>
                                </p>
                            ) : (
                                <div className="space-y-3">
                                    ...
                                    {resolution.candidates.map((c: CustomerCandidate) => (
                                        <label key={c.id} className={`flex items-center gap-3 p-4 rounded-2xl border cursor-pointer transition-all ${selected === c.id ? 'border-primary bg-primary-soft dark:bg-primary/15' : 'border-stone-100 dark:border-stone-800 hover:bg-stone-50 dark:hover:bg-stone-800/40'}`}>
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <p className="text-sm font-bold text-stone-900 dark:text-white truncate">{c.name || c.email}</p>
                                    ...
                                    <label className={`flex items-center gap-3 p-4 rounded-2xl border cursor-pointer transition-all ${selected === 'new' ? 'border-primary bg-primary-soft dark:bg-primary/15' : 'border-stone-100 dark:border-stone-800 hover:bg-stone-50 dark:hover:bg-stone-800/40'}`}>
                                        ...
                                        <span className="text-sm font-bold text-stone-900 dark:text-white">{up.createNewOption}</span>
                                    </label>
                                </div>
                            )}
                            ...
                            {isCreateNew && (
                                <div className="grid grid-cols-2 gap-4 animate-in slide-in-from-top-2 duration-200">
                                    <input required value={customer.name} onChange={e => setCustomer({ ...customer, name: e.target.value })} placeholder={ac.phFirstName} className="h-12 px-5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl focus:ring-4 focus:ring-primary/10 outline-none text-sm font-bold" />
                                    <input value={customer.surname} onChange={e => setCustomer({ ...customer, surname: e.target.value })} placeholder={ac.phLastName} className="h-12 px-5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl focus:ring-4 focus:ring-primary/10 outline-none text-sm font-bold" />
                                    <input required type="email" value={customer.email} onChange={e => setCustomer({ ...customer, email: e.target.value })} placeholder="john@example.com" className="h-12 px-5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl focus:ring-4 focus:ring-primary/10 outline-none text-sm font-bold" />
                                    <input value={customer.taxId} onChange={e => setCustomer({ ...customer, taxId: e.target.value })} placeholder={ac.phTaxId} className="h-12 px-5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl focus:ring-4 focus:ring-primary/10 outline-none text-sm font-bold" />
                                </div>
                            )}
                            ...
                            <div className="flex gap-4">
                                <button onClick={() => setView('upload')} className="flex-1 px-6 py-4 bg-stone-100 dark:bg-stone-800 text-stone-900 dark:text-white rounded-[20px] text-[10px] font-black uppercase tracking-widest hover:bg-stone-200 transition-all">{up.back}</button>
                                <button disabled={!canContinueResolve} onClick={() => setView('confirm')} className="flex-[2] px-6 py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-[20px] text-[10px] font-black uppercase tracking-widest shadow-xl hover:bg-primary dark:hover:bg-mint hover:text-white dark:hover:text-[#1A2420] transition-all disabled:opacity-40">
                    ...
                    {view === 'confirm' && (
                        <div className="space-y-8">
                            <header>
                                ...
                                <h2 className="text-2xl font-black text-stone-900 dark:text-white tracking-tighter mt-2">
                                    {up.confirmTitle} <span className="text-stone-400 italic">{up.confirmAccent}</span>
                                </h2>
                                {presetCustomerName && <p className="text-xs text-stone-400 font-medium mt-1">{presetCustomerName}</p>}
                            </header>
                            ...
                            <label className="flex items-start gap-3 cursor-pointer p-4 rounded-2xl bg-stone-50 dark:bg-stone-800/40 border border-stone-100 dark:border-stone-800">
                                <input type="checkbox" checked={attestedAiConsent} onChange={e => setAttestedAiConsent(e.target.checked)} className="w-5 h-5 mt-0.5 rounded-lg border-slate-300 text-primary focus:ring-primary/30" />
                                <span>
                                    <span className="block text-sm font-black text-stone-900 dark:text-white">{up.consentLabel}</span>
                                    <span className="block text-xs text-stone-400 font-medium mt-0.5">{up.consentDesc}</span>
                                </span>
                            </label>
                    ...
                    {view === 'success' && result && (
                        <div className="py-12 text-center space-y-6">
                            ...
                            <div>
                                <h2 className="text-3xl font-black text-stone-900 dark:text-white tracking-tighter mb-3">{up.successTitle} <span className="text-stone-400 italic">{up.successAccent}</span></h2>
                                <p className="text-stone-500 dark:text-stone-400 font-medium max-w-sm mx-auto">{result.created ? up.successCreatedDesc : up.successAttachedDesc}</p>
                            </div>
                            ...
                            <div className="flex gap-4 pt-2">
                                {result.customerId && (
                                    <button onClick={() => { const id = result.customerId; closeAll(); router.push(`/customers/${id}`) }} className="flex-1 px-6 py-4 bg-stone-100 dark:bg-stone-800 text-stone-900 dark:text-white rounded-[20px] text-[10px] font-black uppercase tracking-widest hover:bg-stone-200 transition-all">{up.viewCustomer}</button>
                                )}
                                <button onClick={closeAll} className="flex-1 px-6 py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-[20px] text-[10px] font-black uppercase tracking-widest shadow-xl hover:bg-primary dark:hover:bg-mint hover:text-white dark:hover:text-[#1A2420] transition-all">{up.done}</button>
                            </div>
                        </div>
                    )}
                </div>
