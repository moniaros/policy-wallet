...
export function GlossaryHint({ hint, className = "" }: { hint: GlossaryHintData; className?: string }) {
    return (
        <details className={`inline-block align-baseline open:block ${className}`}>
            ...
            <span className="mt-1.5 hidden [details[open]>&]:block w-64 max-w-[78vw] rounded-g-control border border-border-subtle bg-surface-sunken p-3 text-g-app-caption font-normal normal-case leading-relaxed tracking-normal text-fg-secondary">
                {hint.definition}{" "}
