...
function formatCoverageValue(value: unknown, locale: string): string | null {
    ...
    if (typeof value === "object") {
        const rawAmount = (value as any).amount
        const rawCurrency = (value as any).currency
        const amount = typeof rawAmount === "number" ? rawAmount : Number(rawAmount)
        if (!Number.isNaN(amount)) {
            const currency = typeof rawCurrency === "string" && rawCurrency.length > 0 ? rawCurrency : "EUR"
...
export function MobilePolicyDetails({
    ...
    const detailsButtonId = `${detailsPanelId}-toggle`
    ...
    const copy = {
        ...
        mainAction: isGreek ? "Κύρια ενέργεια" : "Primary action",
        uploadDocuments: isGreek ? "Ανέβασε έγγραφα" : "Upload documents",
        uploadDocumentsHint: isGreek ? "Ανέβασε 1 έγγραφο για να ξεκινήσεις." : "Upload one document to start.",
        analyzeNow: isGreek ? "Ανάλυσε τώρα" : "Analyze now",
        analyzeNowHint: isGreek ? "Τα έγγραφα είναι έτοιμα για ανάλυση." : "Documents are ready for analysis.",
        analysisInProgress: isGreek ? "Η ανάλυση εκτελείται" : "Analysis in progress",
        analysisInProgressHint: isGreek ? "Η τρέχουσα ανάλυση ολοκληρώνεται. Ανανεώστε σε λίγο." : "Current analysis is running. Refresh shortly.",
        viewUsageBuyMore: isGreek ? "Χρήση AI & αγορά περισσότερων" : "AI usage & buy more",
        viewUsageBuyMoreHint: isGreek ? "Δείτε το υπόλοιπο ή αγοράστε περισσότερα." : "Review usage or buy more credits.",
        editPolicy: isGreek ? "Επεξεργασία συμβολαίου" : "Edit policy",
        sharePolicy: isGreek ? "Κοινοποίηση συμβολαίου" : "Share policy",
        addToWalletAction: isGreek ? "Προσθήκη στο Wallet" : "Add to wallet",
        noIssuesDetected: isGreek ? "Δεν εντοπίστηκαν ενεργά κενά." : "No active gaps detected.",
        usageSnapshot: isGreek ? "Χρήση AI αυτού του μήνα" : "AI usage this month",
        analysesRemaining: isGreek ? "αναλύσεις απομένουν" : "analyses remaining",
        currentCredits: isGreek ? "Τρέχον υπόλοιπο πιστώσεων" : "Current credit balance",
        moreDetailsSection: isGreek ? "Περισσότερες λεπτομέρειες" : "More details",
        collapseDetails: isGreek ? "Απόκρυψη λεπτομερειών" : "Hide details",
        expandDetails: isGreek ? "Εμφάνιση λεπτομερειών" : "Show details",
        fullAnalysis: isGreek ? "Πλήρης ανάλυση κάλυψης" : "Full coverage analysis",
    }
    ...
    const latestDocumentDate = hasDocuments
        ? documents
            .map((doc: any) => parseDate(doc?.uploadedAt))
            .filter(Boolean)
            .sort((a: Date, b: Date) => b.getTime() - a.getTime())[0] || null
        : null
    const lastAnalyzedAt = parseDate(policy?.lastAnalyzedAt)
    const needsAnalysis = hasDocuments && (
        !lastAnalyzedAt ||
        (latestDocumentDate !== null && latestDocumentDate.getTime() > lastAnalyzedAt.getTime())
    )
    const statusIsAnalyzing = policy?.status === "analyzing"
    const analysisInProgress = analyzing || statusIsAnalyzing
    const hasAnalysis = hasDocuments && !needsAnalysis && !statusIsAnalyzing

    const usageCount = aiUsageStats?.count ?? 0
    const usageLimit = aiUsageStats?.limit ?? null
    const usageRemaining = aiUsageStats?.remaining ?? (usageLimit === null ? null : Math.max(usageLimit - usageCount, 0))
    const creditBalance = aiUsageStats?.creditBalance ?? 0
    const usageProgress = usageLimit === null ? 0 : Math.min((usageCount / Math.max(usageLimit, 1)) * 100, 100)

    const analysisPreviewGaps = uniqueGaps.slice(0, 1)
    const showCollaborationPanel = Boolean(showCollaboration) && (
        Boolean(relationshipId) || initialShares.length > 0 || !isOwner
    )
    const hasPeopleDetails = policyPeople.length > 0 || Boolean(insurerContact)
    const hasInsuredItemDetails = Boolean(vehicleInfo?.plateNumber || vehicleInfo?.make || propertyInfo?.address)
    const hasCoverageDetails = allCoverages.length > 0
    const hasRenewalHistory = renewalHistory.length > 0
    const primaryCtaLabel = !hasDocuments
        ? copy.uploadDocuments
        : !hasAnalysis
            ? (analysisInProgress ? copy.analysisInProgress : copy.analyzeNow)
            : copy.viewUsageBuyMore
    const primaryCtaHint = !hasDocuments
        ? copy.uploadDocumentsHint
        : !hasAnalysis
            ? (analysisInProgress ? copy.analysisInProgressHint : copy.analyzeNowHint)
            : copy.viewUsageBuyMoreHint
    const isPrimaryActionDisabled = analysisInProgress && hasDocuments && !hasAnalysis
    ...
    const handlePrimaryAction = async () => {
        if (!hasDocuments) {
            track("upload_cta_click", { source: "mobile_policy_details", policy_id: policy.id })
        ...
        if (!hasAnalysis) {
            track("analyze_cta_click", { source: "mobile_policy_details", policy_id: policy.id })
            await handleAnalyze()
            return
        }

        track("buy_more_cta_click", { source: "mobile_policy_details", policy_id: policy.id })
        router.push("/upgrade?reason=ai_analysis_limit")
    }

    const handleDetailsToggle = () => {
        const willExpand = !detailsExpanded
        setDetailsExpanded(willExpand)
        if (willExpand) {
            track("details_expand_more", { source: "mobile_policy_details", policy_id: policy.id })
        }
    }
                    ...
                    <div className="bg-gradient-to-r from-emerald-500 to-teal-600 p-3.5">
                        ...
                        <div className="flex items-end justify-between">
                            ...
                            {computedDaysLeft !== null && computedDaysLeft >= 0 && computedDaysLeft <= 30 ? (
                                <div className="flex items-center gap-1.5 px-2.5 py-1 bg-amber-400/20 backdrop-blur-sm text-amber-100 rounded-full text-[10px] font-semibold border border-amber-300/30">
                    ...
                    <div className="flex items-center justify-between gap-2 mb-1.5">
                        ...
                        {!hasDocuments ? (
                            <span className="text-[10px] font-semibold text-amber-700 bg-amber-50 border border-amber-200 rounded-full px-2 py-0.5">
                                {isGreek ? "Απαιτείται upload" : "Upload needed"}
                            </span>
                        ) : null}
                    </div>
                    ...
                    {analysisPreviewGaps.length === 0 ? (
                        ...
                        <div className="space-y-2">
                            {analysisPreviewGaps.map((gap: any) => {
                                const explanation = isGreek ? (gap.aiExplanationEl || gap.aiExplanation) : gap.aiExplanation
                                return (
                                    <div key={gap.id} className="rounded-xl border border-red-200 bg-red-50 px-3 py-2.5">
                                        <p className="text-sm font-bold text-red-900">{gap?.definition?.title || copy.gapDetected}</p>
                                        {explanation ? <p className="text-xs text-red-700 mt-1">{truncateText(explanation, 120)}</p> : null}
                                    </div>
                                )
                            })}
                            {uniqueGaps.length > 2 ? (
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex justify-end mt-2.5">
                                                        <button
                                                            ...
                                                            className="flex items-center gap-1.5 px-2.5 py-1 bg-white border border-red-200 rounded-lg text-[11px] font-bold text-red-700 hover:bg-red-50 transition-colors shadow-sm cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-1"
                                                        >
                ...
                <section className="rounded-2xl border border-stone-200 bg-white p-4">
                    ...
                    <div className="space-y-2">
                            {allCoverages.map((coverage: any, index: number) => {
                                ...
                                const deductibleDisplay = formatCoverageValue(coverage?.deductible, locale)
                                const limitDisplay = formatCoverageValue(coverage?.limit, locale)
                                return (
                                    <div
