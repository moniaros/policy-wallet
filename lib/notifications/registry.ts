...
export const NOTIFICATION_EVENTS: Record<string, NotificationEventDefinition> = {
    ...
    green_card_expiry: {
        businessEvent: "A motor policy's green card is inside its expiry window",
        copy: {
            title: { el: "Η Πράσινη Κάρτα σας λήγει σύντομα", en: "Your green card expires soon" },
            message: { el: "Η Πράσινη Κάρτα ενός ασφαλιστηρίου αυτοκινήτου λήγει εντός 30 ημερών. Χωρίς αυτήν δεν αποδεικνύεται η ασφάλιση εκτός Ελλάδας — ζητήστε νέα από τον ασφαλιστή σας.", en: "A motor policy's green card expires within 30 days. Without it, cover cannot be shown abroad — ask your insurer for a new one." },
        },
        triggerCondition: "renewal-check cron finds vehicle.greenCardExpiryDate within 30 days (lib/renewals/green-card.ts)",
        category: "policy",
        priority: "normal",
        channels: FULL_REACH,
        recipients: ["owner"],
        ...
        status: "live",
        emittedBy: "lib/services/renewal.service.ts",
        note: "Spec v2 §14 / §22.3 GREEN_CARD_EXPIRY. The gap rule green_card_expiring fires only at analysis time; this is the calendar-driven reminder. Deduped per policy per expiry date.",
    },
    enfia_season: {
        businessEvent: "The pre-ENFIA season prompt for home policies",
        copy: {
            title: { el: "Ελέγξτε το ασφαλιστήριο κατοικίας σας για τη μείωση ΕΝΦΙΑ", en: "Check your home policy for the ENFIA reduction" },
            message: { el: "Η μείωση ΕΝΦΙΑ για ασφαλισμένες κατοικίες προϋποθέτει κάλυψη πυρκαγιάς, σεισμού και πλημμύρας. Δείτε τι καταγράφει το ασφαλιστήριό σας και ρωτήστε τον ασφαλιστή σας για ό,τι λείπει.", en: "The ENFIA reduction for insured homes requires fire, earthquake and flood cover. See what your policy records and ask your insurer about anything missing." },
        },
        triggerCondition: "enfia-season cron (1 November) finds an in-force home-family policy",
        category: "policy",
        priority: "low",
        channels: FULL_REACH,
        recipients: ["owner"],
        ...
        audit: "notification_event",
        status: "live",
        emittedBy: "lib/services/enfia-season.service.ts",
        note: "Spec v2 §14 / §22.3 ENFIA_SEASON_ALERT. A prompt to CHECK, one per owner per year; eligibility itself is decided only by missing_enfia_components on a policy whose extraction stated all three perils (Law 4223/2013 art. 3 §7ζ, lib/gaps/provenance.ts).",
    },
    renewal_overdue: {
    ...
    benefit_reminder: {
        ...
        copy: {
            title: { el: "Ο ετήσιος έλεγχός σας", en: "Your annual check-up" },
            message: { el: "Ζητήσατε υπενθύμιση για τον ετήσιο έλεγχο που αναφέρει το ασφαλιστήριό σας.", en: "You asked to be reminded about the annual check-up your policy states." },
        },
        triggerCondition: "daily checkup-reminder scan finds a HealthBenefitUsage whose person-chosen remindAt has arrived, not yet reminded, not done and not marked «not relevant»",
        category: "engagement",
        priority: "low",
        channels: FULL_REACH,
        recipients: ["owner"],
        ...
        note: "Prevention brief P1: the PERSON picks whether and when (the 15 January system send was retired 2026-09-24). Changing the date or marking the benefit done cancels by construction — the scan reads live state.",
    },
    daily_nudge: {
        businessEvent: "One general daily habit nudge for a person who opted in",
        copy: {
            title: { el: "Μικρό βήμα για σήμερα", en: "A small step for today" },
            message: { el: "Περπατήστε 15 λεπτά σήμερα.", en: "Take a 15-minute walk today." },
        },
        triggerCondition: "daily-nudge job, once per Athens day, only for users with dailyNudgeOptIn = true; the text is the day's nudge from lib/wellness/nudges.ts",
        category: "engagement",
        priority: "low",
        channels: ["push"],
        recipients: ["owner"],
        ...
        escalation: null,
        retry: NO_RETRY,
        expiresAfterHours: 1 * DAY,
        audit: "notification_event",
        status: "live",
        emittedBy: "lib/services/daily-nudge.service.ts",
        note: "Prevention brief: general habits, the same for everyone, off unless the person turns it on. Honours the person's own outbound switch and monthly ceiling like any engagement send.",
    },
    health_share_received: {
        businessEvent: "A customer shared a health snapshot with this advisor",
        copy: {
        ...
        triggerCondition: "shareHealthWithAdvisor writes an active HealthShare with the person's explicit consent",
        category: "advisory",
        priority: "normal",
        channels: EMAIL_LED,
        recipients: ["advisor"],
        ...
        audit: "notification_event",
        status: "live",
        emittedBy: "app/(protected)/wellness/share-actions.ts",
        note: "Prevention brief P2: the notification carries NO health value — only that a share exists; the advisor reads the snapshot on the customer page, and each view is logged.",
    },
    perk_reminder: {
    ...
    family_member_joined: {
        businessEvent: "A family-wallet invite was accepted",
        copy: {
            title: { el: "Νέο μέλος στο οικογενειακό σας πορτοφόλι", en: "A new member joined your family wallet" },
            message: { el: "Ένα μέλος της οικογένειας βλέπει πλέον τα ασφαλιστήρια που δεν έχετε κρατήσει ιδιωτικά.", en: "A family member now sees the policies you have not kept private." },
        },
        triggerCondition: "applyInviteRedemption consumes a `family` invite (both sides are told)",
        category: "policy",
        priority: "high",
        channels: FULL_REACH,
        recipients: ["owner", "counterparty"],
        transactional: true,
        requiredAction: null,
        escalation: null,
        retry: STANDARD_RETRY,
        ...
        audit: "activity_log",
        status: "live",
        emittedBy: "lib/invites/redeem-invite.ts",
        note: "Spec v2 §13 / §25.3: the moment another person gains sight of a wallet is one both people are entitled to know about. Transactional and unsuppressible.",
    },
    family_member_left: {
        businessEvent: "A family-wallet membership ended",
        copy: {
            title: { el: "Το οικογενειακό πορτοφόλι άλλαξε", en: "Your family wallet changed" },
            message: { el: "Μια πρόσβαση στο οικογενειακό πορτοφόλι έκλεισε.", en: "An access to the family wallet ended." },
        },
        triggerCondition: "endFamilyMembership by the owner (remove) or the member (leave); the OTHER side is told",
        category: "policy",
        priority: "high",
        channels: FULL_REACH,
        recipients: ["counterparty"],
        ...
        audit: "activity_log",
        status: "live",
        emittedBy: "app/(protected)/account/family/actions.ts",
        note: "Spec v2 §13: the inverse of family_member_joined, told to the party who did not act.",
    },
    advisor_assigned: {
