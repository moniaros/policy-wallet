...
6. **English-in-el** — Latin runs of ≥3 letters outside the LEXICON allowlist, measured on **plain string literals only** (bundle, call, ternary, and inline strings that are not object/HTML initializers), with `${…}` **and i18n placeholders `{count}`/`{date}`** stripped first (Round-1 correction: placeholders were counted as Latin in Step 0). The naïve count over all inline initializers (668) is code, not copy. `admin.*` keys and `components/admin` are the staff console, not customer-facing, and are excluded from every metric from Round 1. A literal in which Latin letters outnumber Greek ones is an English string quoting a Greek term (gap-catalogue descriptions, model prompts, schema `.describe()`) — `en` copy with a citation, not English-in-el — and is not counted (Round-1 correction; the first Round-1 measurement counted 177 with them, 71 of which were such lines).
