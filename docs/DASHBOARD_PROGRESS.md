...
| D2 | Probes A2–A6 | test engineer | **A2 done · A6 done** (new build asserts clean: three states only, zero banned/templates/duplicates/shouting; old babel recorded — 11 status words, 29 uppercase runs, 23 duplicates, 1 banned) · **A3 done** (new: zero axe + zero customs, 2 widths × 2 themes; old baseline: 5 findings recorded) — contrast/perf specs pending | tests/audit/{layout,a11y,contrast,performance,consistency}.spec.ts + docs/audit/*-baseline.json | |
