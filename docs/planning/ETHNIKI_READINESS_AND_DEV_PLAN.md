...
>
> **✅ 2026-08-03 (β): +3 κλάδοι live** — motor, health-renewal («life»
> έγγραφο: η ανανέωση του γνωστού Full Health, ταξινομήθηκε σωστά από
> περιεχόμενο), cyber. Backstop αποδεδειγμένο live και στις δύο συμπεριφορές
> (dedup σε πραγματικά AI slugs στο motor· σιωπή στο cyber). Πραγματικό bug →
> migration `constrain_premium_money_scale` (unscaled Decimal αποθήκευσε
> 94.06999… για 94,07 €). Fixtures #5–6 (motor βασική, cyber) με δηλωμένη
> προέλευση. Λεπτομέρειες: docs/STATUS.md.
>
> **☐ Απομένει:** δείγμα ΚΑΤΟΙΚΙΑΣ (ο μόνος rich κλάδος χωρίς πραγματικό
> δείγμα ακόμη) και καθαρό ΖΩΗΣ (το ανεβασμένο ήταν κατά περιεχόμενο υγείας)·
> sweep των υπόλοιπων 19 unscaled Decimal στηλών· `migrate deploy` στην
> παραγωγή (owner).
