...
> **✅ Συμπλήρωμα 2026-07-31 — το χρέος έγινε μετρήσιμο και έπαψε να μεγαλώνει.**
> Το re-ladder δεν γίνεται με ασφάλεια εδώ: αλλάζει διάταξη, και η διάταξη είναι
> ακριβώς ό,τι δεν επαληθεύεται χωρίς browser. Η ίδια η συνεδρία έδωσε την
> απόδειξη — ένα χειριστήριο τριών στηλών πέρασε type-check, lint και 17 δικά
> του tests ενώ ήταν λάθος στα 375px, και το έπιασε φύλακας που **υπήρχε ήδη**.
> Για το re-ladder δεν υπάρχει αντίστοιχος.
>
> Άρα το κομμάτι που **είναι** επαληθεύσιμο, και που το repo το κάνει ήδη για
> χρέη που δεν πληρώνονται αμέσως (bundle size, sold-but-unbuilt): μετρήσιμο
> ratchet. `scripts/reading-size-baseline.json` — **238 `text-xs` + 242
> `text-sm`** στις επιφάνειες απόφασης του ασφαλισμένου (wallet, coverage,
> dashboard). Οι αριθμοί μόνο **κατεβαίνουν**.
>
> Αυστηρό και προς τις δύο κατευθύνσεις: αύξηση = regression, αλλά και
> **baseline που έμεινε ψηλά = χαλαρή πύλη**, που είναι ο τρόπος με τον οποίο
> ένα ratchet παύει σιωπηλά να μετράει. Ένα από τα 5 tests δένει το χρέος με τη
> **λειτουργία μεγάλων γραμμάτων**: όσο μένει απλήρωτο, αυτή είναι που το κάνει
> ανεκτό — αν αφαιρεθεί, οι αριθμοί παύουν να είναι ενόχληση και γίνονται
> φράγμα. Mutations: +1 `text-xs` → κόκκινο· stale-high baseline → κόκκινο.

> **✅ ΜΕΤΡΗΜΕΝΟ 2026-07-31 — και είχα άδικο για το «χωρίς browser».**
> Επί έξι γύρους αυτής της συνεδρίας δήλωνα ότι το 320px sweep είναι αδύνατο
> εδώ. **Δεν το είχα δοκιμάσει ποτέ.** Ο Chromium είναι προεγκατεστημένος
> (`/opt/pw-browsers/chromium`) και το production build σερβίρει τις δημόσιες
> σελίδες **χωρίς ΒΔ** — είναι στατικές.
>
> Νέο `scripts/check-mobile-overflow.mjs` (`npm run check:mobile-overflow`).
> Αποτέλεσμα: **48 φορτώσεις σελίδας, 24 δημόσιες διαδρομές × 2 μεγέθη
> κειμένου, στα 320px — μηδέν οριζόντια υπερχείλιση**, και στο **125%**.
> Δηλαδή ο ισχυρισμός «τα βήματα είναι μετρημένα, τίποτα δεν κόβεται», που τον
> είχα γράψει από συλλογισμό, **επαληθεύτηκε με μέτρηση**.
>
> Ένα πραγματικό εύρημα από την πρώτη εκτέλεση: σάρωνα το `/solutions` επειδή
> είναι στο allowlist του `proxy.ts` — αλλά είναι **prefix**, με μοναδική σελίδα
> το `/solutions/agents`. Η σάρωση ενός prefix μετράει 404 και το λέει επιτυχία·
> γι' αυτό η λίστα είναι πλέον **φύλλα**, όχι prefixes.
>
> Ο ανιχνευτής mutation-tested: ένεση στοιχείου 480px σε πραγματική σελίδα →
> εντοπίζεται. Vacuity floor: αν δεν φορτώσουν όλες οι σελίδες, το script
> **αρνείται** να αναφέρει επιτυχία.
>
> **Δεν μπαίνει στο CI:** το GitHub Actions δεν έχει εγκατεστημένους browsers
> και το repo κρατά σκόπιμα τα E2E εκτός CI. Τοπικό gate, όπως το
> `verify:migrations`.

#### ◐ WP-22 — A11y burn-down σε serious-zero (M) — ΜΕΡΙΚΩΣ 2026-07-31 — R4, R3
...
> **✅ Έγινε — η αναγγελία της ανάλυσης.** Το axe θέλει browser· η κάλυψη
> `aria-live` όμως όχι, και ήταν το ένα κομμάτι του WP που είναι και το πιο
> κρίσιμο για τον χρήστη: **η πιο μακρά αναμονή του προϊόντος ήταν εντελώς
> σιωπηλή**. Ο πίνακας προόδου ανάλυσης στη σελίδα ασφαλιστηρίου δεν είχε
> **κανένα** ARIA — ούτε `role="status"`, ούτε `aria-live`, ούτε
> `role="progressbar"`. Τυφλός ασφαλισμένος που ξεκινούσε ανάλυση δεν
> πληροφορούνταν τίποτα: ούτε ότι άρχισε, ούτε σε ποιο βήμα βρίσκεται, ούτε ότι
> τελείωσε. (Η οθόνη ανεβάσματος και το UsageMeter είχαν ήδη — αυτή όχι.)
>
> Δύο λεπτομέρειες που κρίνουν αν βοηθά ή ενοχλεί:
> - **Το ποσοστό μένει ΕΞΩ από τη live region.** Polite region που περιέχει
>   αριθμό ο οποίος τικάρει αναγγέλλει σε κάθε αλλαγή — χειρότερο από τη σιωπή,
>   καθιστά τη σελίδα άχρηστη όσο τρέχει το run. Η ετικέτα βήματος αλλάζει
>   σπάνια και είναι αυτό που χρειάζεται κανείς· ο αριθμός μένει διαθέσιμος
>   κατ' απαίτηση από το progressbar.
> - **Το `aria-valuenow` δεν κληρονομεί το καλλωπιστικό δάπεδο 8%** της μπάρας.
>   Η μπάρα έχει ελάχιστο πλάτος 8% ώστε ένα run που μόλις ξεκίνησε να φαίνεται
>   ότι ξεκίνησε· η ίδια έκφραση στο ARIA θα ανέφερε 8% πρόοδο σε run που δεν
>   έχει κάνει καμία — μικρό ψέμα, ειπωμένο ακριβώς σε αυτόν που δεν βλέπει τη
>   μπάρα για να το ελέγξει.
>
> 6 tests, mutation-tested και στα δύο σημεία.
>
> **☐ Δεν έγιναν:** axe critical+serious → 0 (θέλει browser), touch targets στη
> ζώνη 13–24px (η αλλαγή είναι διαστάσεων — χωρίς οπτικό έλεγχο σπάει διατάξεις),
> SSR `lang` στα `/en/*` (χρειάζεται per-locale root layouts), dark theme στο
> axe matrix. Οι υπόλοιπες 24 επιφάνειες με spinner χωρίς live region
> καταγράφηκαν — δεν αγγίχτηκαν χωρίς οπτικό έλεγχο ανά μία.
