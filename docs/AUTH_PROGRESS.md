...
| A5 | Google (phase 1) | **LIVE-FLIP IN PROGRESS** (owner enabled the provider in Supabase 2026-08-31; NEXT_PUBLIC_AUTH_GOOGLE=live set in Vercel production; smoke of the round trip follows the redeploy) | Full path shipped: registry → SocialAuthRow → startSocialAuth (server-side URL, PKCE cookies, rate-limited) → signed intent → callback (row birth, role from intent, consent recorded source=oauth_signup, role-mismatch guard, email-verified sync by EMAIL — fixed the id-keyed lookup that no-opped). BLOCKED on owner: Google OAuth client + Supabase provider enablement + prod env flip (ASSUMPTIONS AUTH-05) |
