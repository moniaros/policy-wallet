...
**Production: `16b76a8e`** — the public site's Greek says «ασφαλιστήριο» (#350, deploy run 34708742907, 2026-09-12 17:36Z): ~270 «συμβόλαιο» forms across 43 public files aligned with the app's term, 16 «ασφάλεια»-as-cover lines → «ασφάλιση», «όμως» commas; «ομαδικό συμβόλαιο» and the glossary's «ασφαλιστήριο συμβόλαιο» kept on purpose, product-name «ασφάλεια X» and «Δεν πουλάμε ασφάλειες» untouched, en untouched. Two titles trimmed one word to the 60-char budget (`/`, `/product/health`). Smoked on production: both titles live, zero residual «συμβόλαιο» on `/`, `/product/health`, `/product/motor`, `/compare`; `/lexiko/asfalistirio` (13) and the group guide (32) are the named exceptions; Sentry 0 new issues in the 2h window. Over `d6afd586` — PW-PROVENANCE-01 is closed on production (PR #349, deploy run 34664531255, 2026-09-12 01:24Z). The last four deploys of the series, each smoked: W3-01 (#346 → `e3eac9ee`, deploy run 34661939756, 00:35Z) — the first needs-against-cover comparison with provenance on both sides, published only when the need is stated and the cover confirmed, a question otherwise, rendered on the life line's area detail; R-01 (#347 → `1813dcf1`, deploy run 34662816082, 00:52Z) — passkeys as a second factor over the Supabase session, one gate in the proxy for every ownership pattern, `/auth/step-up`, a settings block — shipped OFF (`PASSKEYS_ENABLED`), the flag is the break-glass; R-02 (#348 → `6d4ac289`, deploy run 34663722041, 01:08Z) — the post-restore re-erasure as the 17th daily cron (`PITR_RESTORE_POINT`), recording `no_restore_point` until a restore happens; R-03 (#349 → `d6afd586`) — the personal-data breach runbook (Art. 33/34, ΑΠΔΠΧ, 72 h), the Art. 33(5) register and a first tabletop drill, guarded so a cited file or route that does not exist fails CI. Evidence: `docs/evidence/provenance-{w3-01,r-01,r-02,r-03}/RESULT.md`; report: `docs/provenance/REPORT.md`. Over `bf71ef69` — the extraction model is no longer asked for a list of beneficiary names (PW-PROVENANCE-01 W5-02, PR #345, deploy run 34660011441, 2026-09-12 00:02Z): `lifeAndInvestment.beneficiaries` stays declared, DEPRECATED, dropped from the prompt block; `beneficiaryCount` and `beneficiaryRelationships` asked for instead; no rule changed (D-P36), catalogue fingerprint untouched; the top-level `beneficiaries[].name` — rendered «as written» on the life card — is a product promise handed to the owner (P-H5). 0 rows on either database carried the path. Anonymous smoke green, no new Sentry group — `docs/evidence/provenance-w5-02/RESULT.md`. Over `43d1f62b` — the extraction model is no longer asked for a third party's name or licence number on an additional driver (PW-PROVENANCE-01 W5-01, PR #344, deploy run 34659023427, 2026-09-11 23:45Z): both driver item schemas gain age band, years licensed and relationship; `namedDriverCount` and `namedDriverRestriction` record what cover turns on; `name`/`licenseNumber` STAY declared, described `DEPRECATED` and dropped from the prompt block by `stripDeprecated` (LOOP §4: a stored shape is never narrowed — D-P33; deleting the keys is the owner's, P-H4); the motor card renders relationship and band. 0 rows on either database carried the array. Anonymous smoke green, no new Sentry group — `docs/evidence/provenance-w5-01/RESULT.md`. Over `b791fce8` — a rule declares the evidence it needs, and a finding below that floor is disclosed, never published (PW-PROVENANCE-01 W2-02, PR #343, deploy run 34657928624, 2026-09-11 23:28Z). **This is the deploy with the visible consequence (P-H3, D-P32): every live production finding whose run recorded no document evidence — all of them, until a policy is re-analysed with confirmed citations — now sits on /protection under «Ευρήματα χωρίς επιβεβαίωση από το έγγραφο» with the disclosure, counts nowhere and is sent nowhere; the dashboard headline gap count should read 0.** The floor derives from the rule's operator class (a silence rule → `policy_silent`, every other → `policy_verified`), overridable per definition with a reason; the verdict (`gap` / `review` / `not_recorded`) is computed on read by the ONE accessor. The demotion was NOT observed signed-in (BL-C2) — asserted on fixtures; anonymous smoke green, no new Sentry group — `docs/evidence/provenance-w2-02/RESULT.md`. Over `2d920c95` — every gap row records what the document was evidence of, per field, in three states (PW-PROVENANCE-01 W2-01, PR #342, deploy run 34656525974, 2026-09-11 23:07Z): `policy_verified` (a value whose citation was found in the text), `policy_asserted` (no citation, a refuted one, or one that could not be checked), `policy_silent` (no value; an empty array counts), plus the weakest, persisted additively as `ruleInputs._evidence` by the ONE writer; `lib/gap-detection.ts` byte-identical (owner-frozen, D-P29). No reader consumes it until W2-02; no row rewritten. Anonymous smoke green, no new Sentry group — `docs/evidence/provenance-w2-01/RESULT.md`. Over `691f8f24` — every citation is verified against the document's own text (PW-PROVENANCE-01 W1-02, PR #341, deploy run 34655235612, 2026-09-11 22:47Z): when an extraction had the probe's local per-page text, each stored `extraction.sources` entry is marked `verified: true|false` with the page it was found on — refuted only within the pages actually read (D-P25), compared folded then letters-only, never fuzzily (D-P26) — and all three providers are guarded to pass the local text. No stored row is rewritten; the mark appears on the next text-native analysis, and W2 is what reads it. Anonymous smoke green, no new Sentry group — `docs/evidence/provenance-w1-02/RESULT.md`. Over `b2449342` — the citation fields cover every field the rules read (PW-PROVENANCE-01 W1-01, PR #340, deploy run 34653889271, 2026-09-11 22:28Z): the 34 `acordData` paths the 50 active rules evaluate are now citation fields, derived from the catalogue and guarded; production already writes `extraction.sources` on 5 of 8 rows, so the next extraction there cites the coverage fields too. **The post-merge CI build died with SIGSEGV in a GitHub runner during static generation and SKIPPED the deploy** — a second way the red-gate trap fires, added to Top risks — re-run green, deploy fired. The series' last four items landed on 2026-09-12 and are recorded at the head of this line — **on W2-02's deploy every live production finding demotes to a disclosed «not confirmed by the document» group until its policy is re-analysed** (P-H3, no grandfathering). Over `ef731076` — the stored `acordData` schema declares the envelope, and the read-site guard checks beneath it (PW-PROVENANCE-01 W0-04, PR #339, deploy run 34652505736, 2026-09-11 22:10Z): `lib/schemas/acord-envelope.ts` declares the seven roots the pipeline writes beside the extraction schema — every field from the writers' literals and the `jsonb_object_keys` sets on both databases, strict, the extraction schema untouched — so the read-site guard now checks 38 more read sites (188 → 237 paths) and a writer adding an undeclared key fails a test. One never-stored read in the clarity rehydration made explicit. Anonymous smoke green, no new Sentry group — `docs/evidence/provenance-w0-04/RESULT.md`. **Also landed: W1-01 (#340 → `b2449342`, CI running): the citation fields now cover the 34 `acordData` paths the 50 rules read; production already writes `extraction.sources` on 5 of 8 rows, so the next extraction cites the coverage fields too.** On branches with green gates: W1-02 (citations verified against the document's text) and W2-01 (three-state document evidence on every gap row — `lib/gap-detection.ts` left byte-identical because it is owner-frozen, D-P29). Over `1c8d98c2` — a text-native PDF can reach the extraction model as page-marked text instead of the whole file, behind a flag (PW-PROVENANCE-01 W0-03, PR #338, deploy run 34649883589, 2026-09-11 21:36Z). `lib/services/ai/extraction-input.ts` decides text-or-file in one place; all three providers build their request through it (guard red-then-green on the real tree); with `EXTRACTION_TEXT_FIRST` unset — the shipped default, and what production has as far as the agent can see — every request is byte-identical to before. **Owner's switch:** set `EXTRACTION_TEXT_FIRST=1` in Vercel (no deploy) and the whole file stops leaving the boundary for text-native PDFs; a scan, a photo or a thin text layer still sends the file with the reason logged. Extraction quality with text input is UNMEASURED on real Greek schedules — that is why it ships off. Anonymous smoke green, no new Sentry group — `docs/evidence/provenance-w0-03/RESULT.md`. Over `cb583bce` — the probe's per-page text travels beside the verdict into the validated document (PW-PROVENANCE-01 W0-02, PR #337, deploy run 34641015833, 2026-09-11 19:54Z): the cheap local read of every upload is no longer thrown away — `PdfProbeResult.pages` keeps each sampled page's raw text, `validateDocumentWithLocalText` hands it out beside the verdict (never on it: the stamp's 12 keys are pinned and a page sentence is asserted absent from the stamp and the activity row), and all three constructor sites put it on `ValidatedAIDocument.localText`, the deep pipeline re-probing on its stamped path. No provider reads it yet; a source guard fails on one that spreads the document whole (red on the real tree with an injected spread). In memory only, no stored shape moved. Anonymous smoke green, no new Sentry group — `docs/evidence/provenance-w0-02/RESULT.md`. **Between the two provenance deploys the flake fix #336 landed as `b77e44a4`** (deploy run 34639825834, 19:37Z): both observed flaky cases in `area-detail-questions` now wait for the state they assert; every CI run since (#337, `cb583bce`, #338) has been green on the first attempt. **W0-03 — text-first extraction behind `EXTRACTION_TEXT_FIRST` — is on PR #338 with a green gate; OFF by default, the owner's switch.** Over `d98bf51f` — every `acordData` read is checked against the schema, and the five wrong readers it found are fixed (PW-PROVENANCE-01 W0-01, PR #335, deploy run 34637008142, 2026-09-11 19:10Z). `Policy.acordData` is `Json?` and every reader casts it, so `tsc` checked nothing; guard `acord-data-read-sites` now derives both universes (188 schema paths; 424 read sites across 66 files) and fails on a read of a path the schema does not declare. **Red first on the real tree, and what it found was live:** the analysis tab read `policy.insurer`, a key the pipeline never wrote, so **every production policy showed an empty insurer tile** — fixed through `displayInsurerName`; three readers of `policy.premium.currency` (schema: `policy.currency`, 0 production rows carry the former) — fixed; four dead fallbacks — deleted; `policyholder`/`insured` written beside the schema on **7 of 8 production rows** — envelope roots and queue item W0-04. Decided by SELECT counts on both databases, not by reading the code (D-P11). `AcordDataSchema` untouched. Anonymous smoke green, no new Sentry group; the fixed tiles are asserted by a render test, not smoked signed-in (BL-C2) — `docs/evidence/provenance-w0-01/RESULT.md`. Over `7f570f51` — the DPIA input pack is generated and the generated compliance records are guarded (PW-PROVENANCE-01 W4-02, PR #334, deploy run 34631981624, 2026-09-11 18:15Z): `docs/compliance/DPIA-INPUTS.md` maps all 714 columns of the 57 personal-data stores to their recorded purpose and basis, states the two Art. 9 routes, and renders a 10-row transfer table from the rows `/subprocessors` publishes with each AI client's constructor read from code (all three: `apiKey` and nothing else — the §14.1 fact, now pinned by a test). **It also closes the gap W4-01 shipped with: `generate-ropa.ts --check` existed and nothing in CI ran it**; guard `generated-compliance-docs-current` now fails the build when either generated document is stale. No runtime surface; anonymous production smoke green, no new Sentry group — `docs/evidence/provenance-w4-02/RESULT.md`. CI passed first attempt this time. Over `568fee72` — the Art. 30 record is generated from the schema (PW-PROVENANCE-01 W4-01, PR #320, deploy run 34627851127, 2026-09-11 17:31Z): 57 personal-data models carry a `/// @ropa` tag, `docs/compliance/ROPA.md` is generated from them and guard `ropa-tags-complete` fails CI on an untagged store. No runtime surface, so the anonymous production smoke (8 public routes 200 in Greek, the auth routes 307 to signin, Playwright 2/2, no Sentry group first seen after the deploy) proves the deploy is live and the product unchanged — `docs/evidence/provenance-w4-01/RESULT.md`. **CI on NEW-UI flaked once more on `area-detail-questions` and SKIPPED the deploy; re-running the failed jobs fired it. Third time in four days — see Top risks.** Over `560a0681` — the pricing surface refuses a purchase it cannot complete (#333, deploy run 34578482913, 2026-09-11). **This closed a LIVE defect, not a hypothetical one:** `/pricing` was advertising €4.99 and €8.99 with a working buy button while `STRIPE_SECRET_KEY` was a TEST key — read from the live page's own payload (`stripeMode:"test"`) — so a visitor could reach a sandbox Stripe page, type a card and land on the success screen having paid nothing. Verified on production after deploy: the paid cards now carry `checkoutUnavailableReason="sandbox_in_production"` and «Οι online αγορές είναι προσωρινά κλειστές» / «Online purchases are paused», the free card is untouched, and every price still renders (€0/€4.99/€8.99/€39/€79 and the agent tiers). **Self-healing:** setting a live key in Vercel restores the buttons with no deploy. Over `53797aee` — the unshared-policy count is the customer's disclosure, not the platform's (#331, halt H-B2 + D-06, deploy run 34454502928, 2026-09-10): with it, PW-BRIDGE-01's last two open questions are answered and the loop's repair phase is complete. The post-merge CI on this commit failed once on the known `area-detail-questions` flake (630/631 files, 7330/7331 tests) and that SKIPPED the deploy entirely — re-running the job turned it green and the deploy then fired. **A red gate does not just report; it silently withholds production.** Over `95c2dbe0` — the registration kill-switch (#332, deploy run 34415772058, 2026-09-09 23:11Z): `ALLOW_REGISTRATIONS` is read on all four account-creation paths, and production's value is `NO`, so signups are CLOSED and the notice is live. Over `c436f177` — the customer's ask becomes a work item (#330, deploy run 34412082119, 22:24Z). Previous: **`892bfd62`** — PW-BRIDGE-01 Queue D measured before designing (#329, deploy run 34328728826, 2026-09-09 08:25Z): three of six candidates closed without building a surface. Over `5f49e9bb` — Queue C closed (#328, 07:04Z). Over `d9e670b1` — Queue C-03/04/05/08, the report saying what the app says (#326/#327, 22:25Z and 23:03Z). Over `cb19406b` — Queue B tiers 2 and 5 (#325, 21:32Z). Over `8852bc50` — tiers 3 and 4 (#324, 20:46Z), with **0 orphan grants** in production. Over `4bcdafa3` — Queue B tier 1 (#323, 19:10Z), verified end to end: generating a branded report created exactly ONE notification row addressed to the POLICY OWNER, in-app, `sent`, naming the actor, dated, deduped per policy per day; a second view added no row. Over `eacc60ad` — Goals 5–6 live and the catalogue aligned (deploy run 34262203801, 18:21Z): production `gap_definitions` 50 rows / 50 active / 14 branches, digest `8045aa4e46935b149e2864f9a20c502a` equal to dev's, `/methodology` renders «50 κανόνες σε 14 κλάδους». Previous: **`55051255`** — Detail of the deployed commit: the bridge residues (#322; deploy run 34158511174, 2026-09-07 20:16Z): C-01b — every residue countdown and both raw expiry windows go through the one lifecycle call (the fact shapes NAME the resolved date; a fourth builder found on the wallet policy page; guard rule (c) on raw windows) and A-01b — the credential column is omitted at the Prisma client (`omit: { user: { password: true } }`, generator `omitApi`; 11 full-row user loads and 9 bare User includes narrowed to selects; guard rules (c)(d)(e)); plus the type-check script's explicit 4 GB heap after CI's ~2 GB default OOMed at the edge. Smoked on production with the owner's policyholder session: /dashboard, /wallet, the expired policy page and /protection render; no app console error. Previous: **`d8827ad7`** — #317 (the compliance docs pack, merged by a parallel session) on top of the B2C home's «Συνοπτική εικόνα» facts row re-laid as a container-stepped fact-cell grid (#321 → `8425f3ea`; deploy run 34140208489, 2026-09-07 15:48Z, carrying both). Smoked on production with the owner's policyholder session at 549px: the card is `@container`, the h2 carries the new recipe, the pill reads «2 δεν αξιολογήθηκαν» and «3.374 € συνολικό ετήσιο ασφάλιστρο», no horizontal scroll, no score fact, no app console error; the desktop widths could NOT be checked on production — the extension cannot resize the owner's window (stays 549) — they hold on the local production build at 1024/1280/1440 (rounds 1–3, `docs/evidence/dashboard-facts/`). Over the four squashes of the 2026-09-07 goal series: bridge C-01/C-02 — the digest and the renewal cron count from the one lifecycle call (#316 → `873eba68`, deploy run 34122411356, 12:32Z; prod `coverage_end_date` backfilled on three rows by SQL, before-values recorded; the two stale open renewal cycles went 2 → 0 with the column repair, 0 of 8 unresolved — SELECT 18:40Z), the register sweep + the placeholder scrub on AI prose (#318 → `85ae3e80`, deploy run 34129168697, 13:45Z; smoked: /protection life tagline «Σιγουρευτείτε», no singular, no placeholder), the agent-side bridge residue A-07/A-11/A-12/A-13/A-14/A-15/A-16/A-17/A-18 (#319 → `04274609`, deploy run 34136272053, 15:07Z; policyholder side smoked on production — the expired policy's review section renders open with the dated provenance line and the pre-plan sentence, no score fact anywhere; the AGENT side is NOT smoked: the owner's browser session offers only Ασφαλισμένος/Διαχειριστής views), and the audit ledger docs (#286 → `f034f9c4`). Housekeeping: #310, #287, #225, #47 closed with notes; #295 kept with a rebase + source-map re-measure comment. Previous: **`21160711`** — the story series' follow-ups (#315, deploy run 34098730747, 2026-09-07 08:10Z: branch content on the ratified term, the summary's recording-finding sentence), over the B2C story series, three squashes on 2026-09-07: the home as a story (#311 → `345d8321`, deploy run 34087702665, 05:44Z), «Καλύψεις & κενά» as a protection story (#314 → `1c9dde12`, deploy run 34089281527, 06:06Z), the mobile menu ☰/X (#313 → `b7811030`, deploy run 34091421340, 06:37Z); each smoked signed-in on production with the owner's session. Previous: **`3c287313`** — PW-CONTENT-01 Goals 1–4 and 7 (PR #308, deploy run 34038640338, 2026-09-06 14:20Z; smoke in `docs/evidence/content-g8/`). Before that: **`7b86805d`** — PW-TRANSPARENCY-02 close-out (PR #307, deploy run 33996579556, 2026-09-05 22:43Z; previous production `e55cf432` = B4 #304, `886ac01f` = R3 #303, `6ff5aebc` = #301, `b0044eaa` = #300, `85088850` = #299). Before the series: **`45013e3f`** — the Document Validation Gate (PR #298), merged and deployed
...
**PW-VOICE-01 — Greek copy audit & rewrite: SERIES CLOSED after three rounds (2026-09-13).** Round 1 #352 → `25dac377` (deployed 2026-09-12 23:15Z, prod-smoked), Round 2 #353 → `8cda0e21`, Round 3 on `feat/voice-01-round3`. 165 `el` strings rewritten under the nine-test rubric — the six advice sentences attributed to the ασφαλιστής, three all-clears that name what they checked, «ασφαλιστικό αποτύπωμα» retired, 46 term locks, 68 anglicisms out (English-in-el 177 → 1), one public score claim («Σκορ Προστασίας 87/100» on `/solutions/agents`) and three timing promises removed, the glossary's score entry moved to the past tense in both languages. Every rewrite within 15 % of its baseline (three listed exemptions); no legal/consent string, no layout. Guards: `tests/unit/voice-guards.test.ts` (8 arms, each with a committed probe proven red) and `tests/measure/dashboard-voice-matrix.spec.ts` (every rewritten string at 320/390/430 against empty/typical/heavy/all-expired; one complete run, 0 clipped, 0 px overflow; later re-runs blocked by the shared dev pooler). **Owner decisions open (`docs/content/HALTS.md`, `docs/content/BACKLOG.md`):** H-V01 onboarding register, H-V04 Greek plan names, H-V05 the product noun, H-V07 the 14-day trial claim, H-V11 the dashboard all-clear slot, **H-V12 two live score renders F2 missed (`AdvisorBookView`, the settings-history timeline) — a code deletion**, H-V13 the agent «Όλα καλά» chip derived without checking analysis coverage. Follow-up under «approved continue» (2026-09-13): H-V12's live score renders deleted or rewritten without the score, dead strings gone, `score-containment` case-insensitive with a literal-score net; H-V13's derivation gains `run_analysis` («Εκκρεμεί ανάλυση») so «Όλα καλά» never covers an unread book. Ledger: `docs/content/PROGRESS.md`.

**Greek copy pass — SHIPPED 2026-09-12 (#350 → `16b76a8e`).** Terminology + register only, everything public. Measured before editing: the prose was careful (ότι/ό,τι, final-ν, enclitic accents all came back clean) — the defect was two words for the policy document with no rule, 186 «συμβόλαιο» vs 163 «ασφαλιστήριο». Rules recorded in `tests/unit/policy-term-asfalistirio.test.ts`, whose public-roots arm was the guard's own documented «deliberately unwidened» gap and is now closed with a per-line two-form exemption and a probe proven red (`tests/fixtures/guard-probes/public-symvolaio.tsx.txt`). Copy freeze moved 215 for 215. Left as-is, noted: `lib/marketing/positioning.ts` claims the homepage renders `STORY.matters`; nothing does. The guard's IN-APP half (`lib/**`, `app/(protected)/**`) stays open as documented.

**PW-PROVENANCE-01 — CLOSED 2026-09-12. Production `d6afd586` (PR #349, deploy run 34664531255, 2026-09-12 01:24Z; smoked anonymously: eight public routes 200 in Greek, three authenticated routes 307, `/privacy` still carries the Art. 33/34 commitment, no Sentry group first seen after the deploy).** Sixteen queue items, every one `done`, each merged to NEW-UI through the GitHub connector, CI-green, auto-deployed and smoked — twelve on 2026-09-11, four on 2026-09-12 (W3-01 `e3eac9ee`, R-01 `1813dcf1`, R-02 `6d4ac289`, R-03 `d6afd586`). Report: `docs/provenance/REPORT.md`; ledgers under `docs/provenance/`; evidence per item under `docs/evidence/provenance-*/RESULT.md`. What is live but inert until the owner acts: text-first extraction (`EXTRACTION_TEXT_FIRST`), passkeys as a second factor (`PASSKEYS_ENABLED`), the PITR re-erasure job (`PITR_RESTORE_POINT`); what is live and VISIBLE: every production finding demotes to «Ευρήματα χωρίς επιβεβαίωση από το έγγραφο» until its policy is re-analysed with confirmed citations (P-H3) — not observed signed-in. Five halts untouched (H-P1…H-P5), two blocked (BL-P1, BL-P2), six owner handoffs (P-H1…P-H6). **Next series:** the owner's pass over `HANDOFF.md` — turn the flags on one at a time, re-analyse one life policy to see the first verified citation and the first published needs-against-cover check, and decide the DEPRECATED keys and the beneficiary names.

**Previous phase note (PW-PROVENANCE-01, in flight 2026-09-11):** **ALLOW_REGISTRATIONS — the registration kill-switch is wired (PR #332, `feat/allow-registrations-flag`, 2026-09-10).** The owner added the variable to Vercel (Production, Preview, Development) and nothing read it. Implemented as a LOCK, not a message: `lib/auth/registration-gate.ts` is the one answer to "may an account be created", and all four doors ask it — `registerUser` (an export of a `"use server"` file, so a callable endpoint with no UI), `/auth/callback` (where the `User` row is born for OAuth AND magic-link), the public `/api/v1/auth/magic-link/request`, and the signup screen. Declared as the flag `auth.allow_registrations` rather than read raw: `parseBooleanEnv` already understands the `YES`/`NO` the owner typed and refuses to read a TYPO as "off", so absent/blank/misspelt means OPEN — the fail direction a front door must have — and the pause is liftable from `/admin/automation/flags` without a redeploy. **Invited customers are exempt** (owner decision): an address with an agent-created phantom row or a live `Invite` still activates, bound to the ADDRESS not the token, so a leaked link opens nothing. The signup screen keeps the form for a `?token=` arrival and otherwise shows a notice that says invitations still work — invited people who type the URL instead of following the link would otherwise be turned away by a rule that exempts them. Guard `account-creation-behind-gate` enumerates creation call sites from the FILESYSTEM and demands an import or a written exemption; **its first probe run PASSED because the probe's own comment mentioned the module path** — the "guard matched a mention inside a comment" failure this repo has recorded twice — so both scans now strip comments and the check requires an `import` statement. Verified in the running app (notice and form, `el`/`en`, both roles, flag off and on; signin untouched), `registerUser` proven never to reach `supabase.auth.signUp` when refused and to reach it when allowed, production build clean with the signup pages still STATIC on a 30s revalidate inherited from the flag cache. **SHIPPED and LIVE: PR #332 merged to NEW-UI as `95c2dbe0`, deploy run 34415772058 success, 2026-09-09 23:11Z. Production `ALLOW_REGISTRATIONS` is `NO` — signups on policywallet.gr are CLOSED right now.** The value was not readable ahead of the merge (`vercel env ls` shows `Encrypted`, and `vercel env pull` redacts — 41 of 55 values came back empty, including plainly non-sensitive ones); it was read AFTER deploy from the page's own serialised prop, the serialised `registrationsOpen` prop reading `false` in the RSC payload of both signup routes — which is also the browser-free way to check it in future. Production smoke: `/auth/signup/policyholder` renders the three-line Greek notice and the «Σύνδεση» button, `/auth/signup/agent` likewise, `?token=…` still renders the full form, `/auth/signin` 200. The form was NOT submitted — creating an account and entering a password are prohibited actions for the agent — so the server-side refusal rests on `tests/unit/register-user-honours-gate.test.ts`, which proves `registerUser` never reaches `supabase.auth.signUp` when refused and does reach it when allowed. Also noted: `/api/v1/auth/magic-link/request` is unreachable today anyway — `proxy.ts` does not allowlist `/api/v1/auth`, so an anonymous POST is 307'd to signin (verified locally) — and has no caller in the app; it was gated regardless.
...
- **PW-VOICE-01 Round 1 shipped (#352 → `25dac377`, deployed, prod-smoked); series closed 2026-09-13 — see Current phase.** 136 Greek strings rewritten under the nine-test rubric (the six advice sentences now attributed to the ασφαλιστής, three all-clears that name what they checked, the coinage «ασφαλιστικό αποτύπωμα» retired, 46 term locks, 34 anglicisms out), every rewrite within 15 % of its baseline (3 listed exemptions), no claim added, no legal/consent string touched. Eight voice guards in `tests/unit/voice-guards.test.ts`, each proven red by a committed probe; English-in-el pinned at 40 under a corrected definition (was 177). Not clean: Round 2 owes the in-app fixture matrix, H-V02/H-V03 verification and the last 40 anglicisms. Owner decisions still open: H-V01 onboarding register, H-V04 plan names, H-V05 the product noun, H-V07 the trial claim (`docs/content/HALTS.md`). Ledger: `docs/content/PROGRESS.md`.

## In progress (2026-09-11)

- **2026-09-11 — PW-PROVENANCE-01 W4-01 is LIVE (#320 → `568fee72`, deploy run 34627851127, 17:31Z).** The first item of the provenance series: the Art. 30 record is derived from `prisma/schema.prisma` (one `/// @ropa` line per model holding personal data — purpose, lawful basis, Art. 9 columns, retention class, export scope, erasure treatment), `scripts/generate-ropa.ts --check` fails when the committed record is stale, and guard `ropa-tags-complete` (11 tests, 5 probes, red on the real tree before green) stops a new personal-data table landing without a recorded purpose and basis. 57 tagged, 0 untagged, 1 model with Art. 9 columns; `unclear` is a valid value so the guard never forces an invented basis (D-P4); the tag is per MODEL, confirmed against a real base change during the four-day drift (D-P6). Verified: local gate on the merged head, CI, the deploy carrying the commit, the anonymous production smoke. Not a DPIA — that is halt H-P2. **W4-02 followed the same day (#334 → `7f570f51`, deploy run 34631981624, 18:15Z): the DPIA input pack — 714 columns classed Art. 9 / subject key / identifier / ordinary, the two Art. 9 routes, the transfer table from the published subprocessor rows with the AI clients' constructor options read from code — and guard `generated-compliance-docs-current`, which is the first thing in CI that fails when a generated compliance document goes stale (W4-01's `--check` was wired to nothing). Two authored maps, both closed: an unknown processor or provider service throws instead of being omitted. Decisions D-P7…D-P9 in PROGRESS. W0-01 followed (#335 → `d98bf51f`, deploy run 34637008142, 19:10Z): the `acordData` read-site guard, the series' blocking prerequisite, is green on the tree — and it found a live defect on the way (see Production). W0-02 followed the same evening (#337 → `cb583bce`, deploy run 34641015833, 19:54Z): the per-page text is carried, in memory, beside the verdict — nothing stored moved, so §4 was not triggered after all (D-P14, D-P15). W0-03 (text-first extraction, flag-gated OFF) shipped the same night (#338 → `1c8d98c2`, deploy run 34649883589, 21:36Z) — **the owner's switch is `EXTRACTION_TEXT_FIRST=1` in Vercel.** W0-04 (#339 → `ef731076`, 22:10Z) and W1-01 (#340 → `b2449342`, 22:28Z) shipped; W1-02 merged (#341 → `691f8f24`). W2-01 and W2-02 are on stacked branches with green gates, waiting for their turn through CI. Next in the queue after W2-01: W2-02 (the evidence floor on a definition — gap only on `policy_verified`, review on `policy_asserted`, «not recorded» wording on `policy_silent` — with the catalogue-enumerating guard; carries a visible consequence, some live findings demote gap → review), then W5-01, W5-02, W3-01, R-01…R-03.** Ledgers: `docs/provenance/{PROGRESS,QUEUES,GUARDS,HALTS,BLOCKED,HANDOFF}.md`.
...
- **BL-P2 — the PITR re-erasure job has never run against a real restore** (`docs/provenance/BLOCKED.md`): none has happened; rehearsing one means restoring production (or a branch of it) to a point in time, the owner's operation. At the next restore rehearsal: set `PITR_RESTORE_POINT`, `POST /api/v1/jobs/pitr-re-erasure` as an admin, attach the `job_runs` row to the DSR evidence doc.
- **BL-P1 — Cloudflare `Workers Builds: policy-wallet` is permanently red and is not a code task**: a `policy-wallet` Worker exists in the Cloudflare account with a git integration onto this repo, while the repo has no Workers project. Delete the Worker or its git integration in the Cloudflare dashboard.
- **BL-C2 — policyholder production smoke**: no policyholder session in the browser; the agent does not sign in with credentials. Every PW-PROVENANCE-01 RESULT.md names the signed-in outcome this leaves unobserved — above all the W2-02 demotion of live findings.
...
- **No MX record on `policywallet.gr`** — dpo@ / info@ / careers@ cannot receive mail (C-H6, GA blocker; owner's DNS). The breach runbook's subject-notification step sends FROM the platform; replies to dpo@ would bounce.
- Partners de-noindexing: Terms §3 qualification + IDD opinion (legal — `docs/legal-review-queue.md`; also halt H-P5).
- Guides 6–8: citation debt (A-03: citation-clean or not at all).

## Top risks, ranked

1. **AI providers are not regionally pinned and no zero-retention setting is applied in code** (halt H-P1). Complete policy documents — Art. 9 health content included — go to the global Gemini AI Studio endpoint under whatever the account defaults are. Partial mitigation shipped OFF: `EXTRACTION_TEXT_FIRST=1` sends a text-native PDF as page-marked text instead of the whole file (W0-03); the DPIA input pack pins each client's constructor to `apiKey` only.
2. **Every production finding renders as «Ευρήματα χωρίς επιβεβαίωση από το έγγραφο» until its policy is re-analysed with confirmed citations** (W2-02, P-H3, no grandfathering). The first impression of every wallet is now zero published gaps and a disclosed group — correct by the evidence rule, and NOT observed signed-in (BL-C2). The first re-analysis of a life policy is also the first verified citation and the first needs-against-cover verdict (W3-01); until then that block is a question.
3. **Engine plans from the database, composition fingerprints the repo.** Aligned on both databases since 2026-09-08 (50 rules, `15fa2758cebeaecc`); any catalogue change without the matching alignment makes every new production run render the stale-catalogue line and no new rule fires. Adding `beneficiaryCount` to the beneficiary rules would be such a change (D-P36 kept them untouched).
4. **A red CI gate silently SKIPS the deploy** — and a GitHub runner SIGSEGV during static generation is a second way it fires (W1-01, run 34652698007). The `area-detail-questions` flake itself was fixed in #336 and every run since has been green first attempt; the trap shape remains. `gh run rerun <id> --failed` is the recovery.
5. **Passkeys, once the owner turns them on, have one recovery path: the flag.** `PASSKEYS_ENABLED=1` enrols; a lost authenticator locks the owner out of every protected page until the flag is unset and redeployed (D-P41, P-H6). Enrol two authenticators before relying on one.
6. **Marketing routes ship the app bundle** (mobile LCP 4.8s vs the 2.0s budget; `docs/perf-report.md`).
7. **`/en/*` server HTML says `lang="el"`** until an inline script runs (root-layout limitation, documented in `app/layout.tsx`; Lighthouse SEO 100 because it reads the hydrated DOM).
...
1. **Owner — the PW-PROVENANCE-01 handoff pass (`docs/provenance/HANDOFF.md`, P-H1…P-H6):** open /protection signed in and confirm the demotion reads as intended (P-H3); re-analyse one life policy and watch `extraction.sources[].verified` and the «Ανάγκη έναντι κάλυψης» block turn from question to verdict; decide the DEPRECATED driver keys (P-H4) and the beneficiary names as a product promise (P-H5); set `PASSKEYS_ENABLED=1` and enrol two authenticators (P-H6); set `EXTRACTION_TEXT_FIRST=1` when ready to measure text-first quality on real Greek schedules.
2. **Owner — counsel and staffing:** the five halts (`docs/provenance/HALTS.md` H-P1…H-P5: EU pinning / zero retention, the DPIA itself, agent-attested consent, the single-operator DSR model, the Terms §3 / IDD / Art. 9 wording items), legal sign-off of PROVENANCE-REVIEW.md (C-H2), the MX record (C-H6).
3. **Next code series, whichever the owner picks:** (a) the rotation half of the breach drill on a preview environment with a disposable key, appended to `docs/operations/PERSONAL_DATA_BREACH_DRILL_EVIDENCE_2026-09.md`; (b) the restore rehearsal that closes BL-P2; (c) ratify a `need_vs_limit` operator for the frozen engine so W3-01's comparison can become a rule with rows (D-P38), and a `count_zero` operator so the beneficiary count can feed `no_beneficiaries_recorded` (D-P36).
