...
**⚠ Blocked on permissions, ready to run:** (1) prod plan canonicalization —
`scratchpad/canonicalize-plans.sql` statements 1–7 (behavior-neutral by construction);
(2) prod balance repair: `UPDATE token_balances SET used_tokens=0 WHERE user_id=(SELECT
user_id FROM users WHERE email='moniaros@gmail.com')` — after which `verify:plans` against
prod should print 7/7 OK.
