...
## Done (recent)
- **Data-flows / extensibility / cybersec deck (10 Jul):** 12-slide Greek deck in `docs/presentations/2026-07-data-flows-extensibility-cybersec/` + 2 Lucidchart diagrams (architecture, AI-pipeline sequence) + Postman collection "PolicyWallet API v1 — Integration Surface" (team workspace). Docs-only change.
- **App-wide UI repaint (branch `ui-consistency-repaint`, 10 Jul):** logged-in app aligned to the marketing-widget design language — token layer retargeted (#1FDC86 emerald → #29685B deep green, dark mode = mint #89D9B2 with #1A2420 text on fills), Inter app-wide via next/font, ~150 files swept per a status-vs-brand mapping table (amber/red/info semantics preserved, brand gradients flattened, AI badges kept on #4F46E5), auth pages gained dark mode, MASTER.md rewritten (was stale gold/purple), email templates retinted, emails/chrome off teal-500. All guardrails + 145/145 tests + build green; dark flip verified via computed styles. NOT yet deployed.
