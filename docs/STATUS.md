...
`scripts/verify-pipeline-live.ts` με πραγματικό GEMINI_API_KEY (τοπικά, εκτός
git) πάνω στο πραγματικό ασφαλιστήριο γενικής αστικής ευθύνης. **Και τα 5
golden πεδία ακριβή:** «Η ΕΘΝΙΚΗ» (πλήρης επωνυμία), 140014/2, κλάδος
**liability** (σωστή κανονικοποίηση), **2026-06-30 → 2027-06-29** (οι dotted
ημερομηνίες του εγγράφου), ασφάλιστρο **100** (το σύνολο, όχι το καθαρό).
Η περίληψη αναγνώρισε «διαχειριστή πολυκατοικίας στη Γλυφάδα» — τη διεύθυνση
κινδύνου. Ένα gap ανέφερε την **πραγματική απαλλαγή των 350 €** στα ελληνικά.

**Η ανθεκτικότητα δούλεψε ζωντανά:** attempt 1 εξαγωγής απέτυχε
(SCHEMA_MISMATCH, «No object generated») → attempt 2 πέτυχε· 9/9 βήματα,
100% success pct. **Metering ακριβές (WP-15):** 3 γραμμές token_usage =
45.061 tokens, ίσο με το actual_total_tokens του run. Score envelope v2,
11 δίγλωσσα AI gaps, μηδέν taxonomy_* (σωστή σιωπή για κλάδο χωρίς ταξινομία),
μηδέν DSL (κανένας seeded liability κανόνας — σωστό).

**Owner:** το κλειδί δόθηκε μέσα από το chat → **rotate άμεσα** και προσθήκη
στο CCR environment / Vercel env vars για μόνιμη χρήση.
