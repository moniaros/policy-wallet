...
- **Agent-side production smoke of #319** (A-07/A-13/A-17 on `/customers/[id]/policy/[policyId]`): the owner's browser session is the policyholder and its «Προβολή ως» switcher offers only Ασφαλισμένος/Διαχειριστής — needs the owner's agent account signed in (the agent does not sign in with credentials).
