...
## Done (recent)
- **Architecture review + first quality refactor** (`docs/audits/architecture-review.md`): reverse-engineered the full system (data model, golden-path data flow, AI pipeline, auth/API, services, frontend); cataloged problems split into **broken/insecure** (role substring-match, inconsistent API auth/response contracts, token-reservation leak, dedup race) vs **quality/maintainability** (2,481-line orchestrator, 3 near-duplicate AI providers ~55%, `policy.service` God object). Ranked a behaviour-preserving refactor plan. **Shipped step 5.1:** extracted `toISODate()` into `lib/constants/time.ts`, replaced 15/17 `.toISOString().split('T')[0]` sites (2 client-side deferred to avoid pre-existing i18n debt). Guardrails all green locally — including full type-check + unit suite (worked around the sandbox Prisma-engine block by fetching engines via curl). **Shipped step 5.2 (partial):** `lib/services/authorization.ts` `resolvePolicyAccess()` now backs the 3 owner-or-grant checks in `gap-analysis.service.ts` (semantics + query ordering preserved; the gap mutations correctly stay relationship-blind), locked with `tests/unit/authorization.test.ts`. Unit suite 116 pass. Now extended to **9 sites** (gap-analysis ×3, both analysis-runs routes, wallet/actions ×4) via an optional `policyScopeId` that preserves each site's exact grant-query breadth; share-revoke flows that need the grant record are intentionally left. Unit suite **118 pass**. Pure owner-or-grant access checks are now centralized. _Branch `claude/codebase-architecture-review-jx2um9`; commits 10db0fa, 41a6b42, e33bc54._ **Shipped §5.3 (with a course-correction):** investigated the "~55% duplicate AI providers" lead and **rejected** the base-class merge — the per-provider Zod schemas/prompts are intentionally tuned and feed model output, so merging them would change behaviour. Extracted only the byte-identical `buildMessageParts()` payload builder (6 sites) into `shared-utils.ts`; message-payload tests stay green. _Commit 56f4047._ **Next:** full el/en i18n key-parity guard (current parity test covers only 6 namespaces) + a `safeFireAndForget` helper for the 6 silent `.catch(()=>{})` sites.
