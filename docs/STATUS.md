...
6. **`tests/unit/area-detail-questions.test.tsx` is a deploy-blocking flake.** Three red CI runs on NEW-UI in four days (#331; #320's PR run and its post-merge run), each a different timing case of the same file, and every red gate SKIPS the deploy — `deploy.yml` only fires on a green `workflow_run`. The recovery is «re-run failed jobs», which happens only while someone is watching. Either the file gets a deterministic clock and awaited transitions, or one day production silently stays stale. **A fourth flake on 2026-09-11 (#335's PR run) made it five; PR #336 (`test/area-detail-questions-deterministic`) makes both observed cases wait for the state they assert — 8 of 8 local runs green, one uncaptured failure in an earlier loop stated in the PR. MERGED as `b77e44a4` (2026-09-11); the file has passed every CI run since on the first attempt. Downgrade this risk when a week of runs agrees. A SECOND shape of the same trap on 2026-09-11 22:20Z: the post-merge build of `b2449342` died with `SIGSEGV` in the runner's static-generation worker — not a test, not a code error — and the deploy was skipped just the same. `deploy.yml` fires only on a green `workflow_run`; anything red, for any reason, withholds production until a person re-runs it.**
