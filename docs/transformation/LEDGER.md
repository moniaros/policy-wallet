...
| WL-11 | The check-up's terms (frequency, cap, tests, network, waiting, conditions) inside the Benefit Reminder | fact | **KEEP** | only what `health.checkup` states; a term without a verified citation is marked «δεν εντοπίστηκε αυτούσιο στο έγγραφο»; missing frequency or cap → «Ό,τι δεν αναγράφεται εδώ χρειάζεται επιβεβαίωση» | — |
