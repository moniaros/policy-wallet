#!/bin/bash
# Anonymous production smoke for PW-PROVENANCE-01 W0-01 (the acordData read-site guard + five reader fixes).
# The guard runs at test time. The five reader fixes change what a SIGNED-IN policy page and analysis tab
# render (insurer tile, premium currency); an anonymous smoke cannot see that, so it proves only that the
# deploy carrying the item is live and the public product unchanged. The rendered outcome is asserted by
# the unit render test named in RESULT.md.
# Re-runnable; writes nothing.
#
#   bash docs/evidence/provenance-w0-01/prod-smoke.sh
