#!/bin/bash
# Anonymous production smoke for PW-PROVENANCE-01 W4-02 (the DPIA input pack, generated; the currency guard).
# The item has no runtime surface: the smoke proves the deploy that carries it is live and that the
# public product is unchanged by it (the pack is a repository document; the generator and guard run at
# build/test time only). Re-runnable; writes nothing.
#
#   bash docs/evidence/provenance-w4-02/prod-smoke.sh
...
curl -s -o /dev/null -w '%{http_code} /docs/compliance/DPIA-INPUTS.md\n' -A "pw-smoke" "$H/docs/compliance/DPIA-INPUTS.md"
