#!/bin/bash
# Anonymous production smoke for PW-PROVENANCE-01 W0-03 (text-first extraction, flag-gated OFF).
# The item ships behind EXTRACTION_TEXT_FIRST (default off): with the flag off every provider request is
# byte-identical to before, so production behaviour is unchanged by this deploy until the owner sets the
# variable. An anonymous smoke proves only that the deploy carrying the item is live and the public product
# unchanged; the request shape under the flag is asserted by the payload tests named in RESULT.md.
# Re-runnable; writes nothing.
#
#   bash docs/evidence/provenance-w0-03/prod-smoke.sh
