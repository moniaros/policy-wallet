    ...
    return (
        <LoBPageShell activeNav="none">
            <article className="mx-auto max-w-[760px] px-6 pb-16 md:px-12">
                ...
                <h1 className="mb-6 text-[36px] font-semibold leading-[1.1] tracking-[-0.03em] text-[#0F172A] md:text-[44px] dark:text-white">
                    {entry.term[lang]}
                </h1>

                {/* Answer-first block — the extractable definition (AEO) */}
                <p className="mb-8 border-l-2 border-[#29685B] pl-5 text-[19px] leading-relaxed text-[#0F172A] dark:text-slate-200">
                ...
                <div className="space-y-5 text-[17px] leading-relaxed text-[#334155] dark:text-slate-300">
                ...
                <div className="mt-10 rounded-2xl border border-[#E2E8F0] bg-[#F8FAFC] p-6 dark:border-slate-800 dark:bg-slate-900">
                    <div className="mb-3 flex items-center gap-2 text-[15px] font-semibold text-[#0F172A] dark:text-white">
                        <ShieldCheck className="h-5 w-5 text-[#29685B]" />
                        {t("Πώς το ελέγχετε στο συμβόλαιό σας", "How to check it in your policy")}
                    </div>
                    <p className="text-[16px] leading-relaxed text-[#475569] dark:text-slate-300">
                ...
                {entry.faq.length > 0 && (
                    <section className="mt-12">
                        <h2 className="mb-5 text-[24px] font-semibold tracking-tight text-[#0F172A] dark:text-white">
                        ...
                        <div className="divide-y divide-[#E2E8F0] dark:divide-slate-800">
                            {entry.faq.map((item, index) => (
                                <div key={index} className="py-5">
                                    <h3 className="mb-2 text-[17px] font-semibold text-[#0F172A] dark:text-slate-100">
                                        {item.question[lang]}
                                    </h3>
                                    <p className="text-[16px] leading-relaxed text-[#475569] dark:text-slate-300">
                                        {item.answer[lang]}
                ...
                {entry.related && entry.related.length > 0 && (
                    <section className="mt-12">
                        <h2 className="mb-4 text-[15px] font-semibold uppercase tracking-widest text-[#64748B]">
                            {t("Σχετικά", "Related")}
                        </h2>
                        <div className="flex flex-col gap-2.5">
                            {entry.related.map((link, index) => (
                                <Link
                                    key={index}
                                    href={l(link.href)}
                                    className="group inline-flex items-center gap-2 text-[16px] font-medium text-[#29685B] hover:underline"
                                >
                ...
                {/* Honesty line */}
                <p className="mt-12 flex items-start gap-2 text-[13px] leading-relaxed text-[#94A3B8]">
