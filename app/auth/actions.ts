...
export async function registerUser(formData: FormData) {
    ...
    try {
        ...
        // Get verification URL from Supabase (if available in response)
        // Otherwise construct a generic one
        const verificationUrl = authData.user.confirmation_sent_at
            ? `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/auth/verify?email=${encodeURIComponent(email)}`
            : '#'
