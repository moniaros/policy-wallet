    ...
    return (
        <div className="max-w-5xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
            ...
            <nav className="flex items-center gap-2 mb-8 text-sm font-medium">
                <Link href="/customers" className="text-neutral-400 hover:text-primary dark:hover:text-mint transition-colors">{pd.breadcrumbCustomers}</Link>
                <svg className="w-4 h-4 text-neutral-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" />
                </svg>
                {customer ? (
                    <Link href={`/customers/${customerId}`} className="text-neutral-400 hover:text-primary dark:hover:text-mint transition-colors">
                        {customer.name}
                    </Link>
                ) : (
                    <span className="text-neutral-400">{pd.loading}</span>
                )}
                <svg className="w-4 h-4 text-neutral-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" />
                </svg>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="bg-white dark:bg-neutral-800 rounded-3xl shadow-sm border border-neutral-200 dark:border-neutral-700 overflow-hidden">
                        <div className="p-8 border-b border-neutral-100 dark:border-neutral-700">
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                ...
                                <div className="bg-neutral-50 dark:bg-neutral-900 p-6 rounded-2xl border border-neutral-100 dark:border-neutral-700 text-center md:min-w-[200px]">
                                    <p className="text-xs font-bold text-neutral-400 uppercase tracking-widest mb-1">{pd.annualPremium}</p>
                        ...
                        <div className="p-8">
                            <h2 className="text-sm font-black text-neutral-400 uppercase tracking-widest mb-6">{pd.coverageHighlights}</h2>
                    ...
                    <div className="bg-neutral-50 dark:bg-neutral-900/30 rounded-3xl p-8 border border-neutral-100 dark:border-neutral-800">
                        <h2 className="text-sm font-black text-neutral-400 uppercase tracking-widest mb-4">{pd.summary}</h2>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white dark:bg-neutral-800 rounded-3xl p-6 shadow-sm border border-neutral-200 dark:border-neutral-700">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-sm font-black text-neutral-400 uppercase tracking-widest">{pd.documents}</h3>
                        </div>

                        {policy.documents.length === 0 ? (
                            <div className="text-center py-8">
                                <p className="text-xs text-neutral-400 italic">{pd.noFiles}</p>
                            </div>
                        ) : (
                            <ul className="space-y-4">
                                {policy.documents.map((doc: any) => (
                                    <li key={doc.id}>
                                        <a href={doc.fileUrl} target="_blank" rel="noreferrer" className="flex items-center gap-3 p-3 rounded-2xl hover:bg-neutral-50 dark:hover:bg-neutral-700/50 group transition-all">
                                            <div className="w-10 h-10 rounded-xl bg-neutral-100 dark:bg-neutral-900 flex items-center justify-center text-neutral-400 group-hover:text-primary dark:group-hover:text-mint transition-colors">
                                            ...
                                            <div className="overflow-hidden">
                                                ...
                                                <p className="text-xs text-neutral-400 uppercase tracking-widest">{pd.contract}</p>
                                            </div>
                                        </a>
                                    </li>
                                ))}
