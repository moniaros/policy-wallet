...
export function CoverageChecklist({ lines, stateLabels, className }: { lines: ChecklistLine[]; stateLabels: Record<ChecklistState, string>; className?: string }) {
    return (
        <ul className={cn("overflow-hidden rounded-g-card border border-border-hair bg-surface-raised [&>li+li]:border-t [&>li+li]:border-border-hair [&>li+li]:[border-top-width:0.5px]", className)}>
            {lines.map((l) => (
